    ...
    $.fn.tabullet = function (options) {
        ...
        $(metadata).each(function (i, v) {
            ...
            if (v.type === 'select') {
                ...
                selectHtml += '</select>';
                var td = $("<td/>").html(selectHtml).attr('data-tabullet-type', 'select').appendTo(insertRow);
                return;
            }
            if (v.readonly !== 'true') {
                            ...
                            if ($(rv).attr('data-tabullet-type') === 'select') {
                                ...
                                for (var i = 0; i < select.length; i++) {
                                    if (select[i].value === v[mapName]) {
                                        selectHtml += '<option selected>' + select[i].value + '</option>'
                                    } else {
                                        selectHtml += '<option>' + select[i].value + '</option>'
                                    }
                                }
                                selectHtml += '</select>';
                                $(rv).html(selectHtml).attr('data-tabullet-type', 'select');
                                return;
                            }
